...
export default function TimeframeRow({ data, colorOf, onSelect }: Props) {
  const d1m = dailyComposite(data, '1m')
  const d2w = dailyComposite(data, '2w')
  const intra = intradayComposite(data)

  const views = CARD.map((c) => {
    if (c.tf === '1w') return { ...c, labels: intra.labels, series: intra.series }
    const d = c.tf === '1m' ? d1m : d2w
    return { ...c, labels: d.labels, series: d.series }
  })
